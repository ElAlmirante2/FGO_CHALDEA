var canvas, context;
	...
	var AllCategoryNUM = {
		"tw": [10, 9, 8, 10, 11, 9, 10, 5, 3, 5, 2, 2],
		"jp": [14, 11, 11, 12, 13, 10, 12, 7, 5, 6, 6, 3]
	};
	...
	function init(state = 0){
		...
		context.fillText("This image was original made by mgneko.github.io", 190 + marginLeft, canvas.height - 25);
	}
	...
	function fillTotalText(){
		...
		context.fillStyle = font_color;
		context.fillText("5*: "+ totalHave + "/" + total,
			canvas.width - 200,
			canvas.height - 90);
		context.fillText("Porcentage: " + percent.toFixed(2) + "%",
			canvas.width - 200,
			canvas.height - 70);
		context.fillText("Total 5*: " + totalNP,
			canvas.width - 200,
			canvas.height - 50);
	}
